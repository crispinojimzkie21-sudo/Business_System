...

{
...

    {
        ...
        foreach ($request->loads as $loadData) {
            ...
            if (!$eloadNumberRecord) {
                ...
                $eloadNumberRecord = EloadNumber::create([
                    ...
                    'provider' => $loadData['network'] . ' Telecom',

                    'number_type' => 'Mobile',

                    'description' => $loadData['network'] . ' mobile number',
            ...
            $transaction = EloadTransaction::create([
                ...
                'original_price' => $loadData['price'],

                'amount' => $loadData['price'],

                'customer_name' => 'Walk-in Customer',

                'customer_mobile' => $formattedNumber,

                'network' => $eload->network ?? $loadData['network'],

                'provider' => $eload->provider ?? $loadData['network'] . ' Telecom',
        ...
        return redirect()->route('admin.eload.transactions.history')
    ...
    // ==================== ADMIN ASSISTANT METHODS ====================



    /**

     * Show add load form for Admin Assistant.

     */

    public function adminAddLoad()

    {
        ...
        return view('eload.admin-add-load', compact('eloads', 'eloadNumbers', 'numbersByNetwork'));
     ...
     * Show add multiple loads form for Admin Assistant.

     */

    public function adminAddLoadMultiple()

    {

        return view('eload.admin-add-load-multiple');
     ...
     * Process add load transaction for Admin Assistant.

     */

    public function adminProcessLoad(Request $request)
     ...
     * Process multiple load transactions for Admin Assistant.

     */

    public function adminProcessMultipleLoads(Request $request)
