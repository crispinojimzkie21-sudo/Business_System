    ...
    <div class="max-w-4xl mx-auto p-6">
        ...
        <header class="flex items-center justify-between mb-8">
            <div>
                ...
                <p class200 text-sm mt="text-blue--1">Update employee information</p>
            </div>
            <a href="{{ url('/admin/list') }}" class="px-4 py-2 text-blue-200 hover:bg-blue-900/30 rounded">← Back to List</a>
        </header>
        ...
        <div class="bg-black/60 p-6 rounded-lg border border-blue-900/30">
            <form method="POST" action="{{ route('admin.update', $user->id) }}" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-blue-300 mb-2">
                            <input type="checkbox" name="access_enabled" {{ $user->access_enabled ? 'checked' : '' }} class="mr-2">
                            System Access Enabled
                        </label>
                        <p class="text-xs text-gray-400 mt-1">Uncheck to block login access (Super Admin control)</p>
                    </div>
                </div>
                ...
                <div class="flex gap-4">
                    ...
                    <a href="{{ url('/admin/list') }}" class="px-6 py-3 bg-gray-700 hover:bg-gray-600 text-white font-semibold rounded-md transition-colors">
                        Cancel
                    </a>
                </div>
            </form>
            ...
            <div class="mt-8 pt-6 border-t border-blue-900/30">
                ...
                <form method="POST" action="{{ route('admin.destroy', $user->id) }}" onsubmit="return confirm('Are you sure you want to delete this employee? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
